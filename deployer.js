...
function handleCli(conf){
	...
	// conf.no_gui = isNA(cli.opts()["terminal"]) ? false : cli.opts()["terminal"] && false;// Remove && false
...
function run(dry){
	...
	async.series([
		...
		for(var command in deployer.config.project.commands){
			try{
				deployer.config.actionObjects[command] = new Command(deployer.config.project.commands[command]);
		...
		if(dry){
			...
			var initialCmd = deployer.config.actionObjects[deployer.config.action];
			if(initialCmd){
				console.log(deployer.config.actionObjects, deployer.config.action);
...
function runPermanentCli(){
	...
	for(var command in deployer.config.project.commands){
		...
		if(!tmpcmd.awake){
			...
			tmpcli.action((function(){
				var cmd = command;
				return function(){
					if(clicb != null && clicb.constructor.name == "Function"){ return deployer.config.actionObjects[cmd].setArgumentsGlobal(merge(true, deployer.config.project.args)).execute(function(){
						//load();
						clicb();
					});
																			 }
	...
	return async.doUntil(
		function(cb){
			rl.on('close',function(){ // When the inner rl loses focus...
				process.exit(0);
			});
			rl.setPrompt(colour["green"](colour["bold"]("    => ")));
			rl.prompt();
			rl.on('line', function(data){ // When pressing "enter"
